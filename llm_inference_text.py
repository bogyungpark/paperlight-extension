...
--------
  ...
  cd ~/Desktop/CODE/mine/paperlight-extension

  # 1. 모델 목록 (서버 ping)
  python3 llm_inference_text.py --list-models
  #   Qwen/Qwen2.5-32B-Instruct-AWQ

  # 2. 한 줄 질문 (스트리밍 + 응답 시간 측정)
  python3 llm_inference_text.py --timing "이 논문의 핵심 contribution이 뭐야?"
  # 2. 한 줄 질문 (스트리밍 + 응답 시간 측정)
  python3 llm_inference_text.py --timing "이 논문의 핵심 contribution이 뭐야?"
  ...
  # 3. 시스템 프롬프트 + 온도 조절
  python3 llm_inference_text.py \
    -s "You are a meticulous research-paper assistant." \
    -t 0.2 \
    "Explain Mixture of Experts in 3 sentences"

  # 4. 대화형 (멀티턴 — 'exit' / 'reset' 명령)
  python3 llm_inference_text.py -i

  # 5. stdin 파이프
  echo "이 문장을 영어로 번역해줘: 강화학습은..." | python3 llm_inference_text.py

  # 6. 다른 endpoint (예: OpenAI 진짜)
  PAPERLIGHT_URL=https://api.openai.com/v1 \
  PAPERLIGHT_MODEL=gpt-4o-mini \
  OPENAI_API_KEY=sk-... \
    python3 llm_inference_text.py "hi"
  ...
  │ -s            │ system 프롬프트                                             │
  ├───────────────┼─────────────────────────────────────────────────────────────┤
  │ -u / -m       │ URL / 모델 override (또는 PAPERLIGHT_URL, PAPERLIGHT_MODEL) │
  ├───────────────┼─────────────────────────────────────────────────────────────┤
  │ -t / -n       │ temperature / max_tokens                                    │
  ├───────────────┼─────────────────────────────────────────────────────────────┤
  │ -i            │ 대화형 REPL                                                 │
  ├───────────────┼─────────────────────────────────────────────────────────────┤
  │ --no-stream   │ 완성된 응답을 한 번에 출력                                  │
  ├───────────────┼─────────────────────────────────────────────────────────────┤
  │ --timing      │ 처리량 출력 (방금 봤듯 55 ch/s)                             │
  ├───────────────┼─────────────────────────────────────────────────────────────┤
  │ --list-models │ 엔드포인트가 노출한 모델 목록                               │
  └───────────────┴─────────────────────────────────────────────────────────────┘
